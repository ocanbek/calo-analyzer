...
def test_normalization_correct():
    ...
    result = process_calorimetry_data(
        df, "time_s", "hf_mw", time_unit="s", heat_flow_unit="mW",
        cement_mass_g=5.0, integration_start_h=0.0,
    )
    np.testing.assert_allclose(
        result.df["Normalized heat flow (mW/g)"], [2.0, 4.0, 6.0]
    )


def test_missing_and_non_numeric_rows_dropped_and_counted():
    df = pd.DataFrame(
        {"t": [0, 600, "bad", 1800], "q": [1.0, 2.0, 3.0, np.nan]}
    )
    result = process_calorimetry_data(
        df, "t", "q", cement_mass_g=1.0, integration_start_h=0.0
    )
...
def test_cumulative_heat_column_produced_via_integration():
    ...
    result = process_calorimetry_data(
        df, "t", "q", cement_mass_g=5.0, integration_start_h=0.0
    )
    # normalized heat flow constant at 2 mW/g -> cumulative = 2 * t_h * 3.6
    np.testing.assert_allclose(
        result.df["Cumulative heat (J/g)"], [0.0, 3.6, 7.2]
    )
